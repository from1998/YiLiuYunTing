    ...
    <el-container class="container">
      ...
      <div v-for="(item,index) in resdata" :key="item.id" class="block">
        ...
        <el-container class="container">
          <el-main class="main">
            <!-- 图片 -->
            <el-image :src="item.carRecord!==null?(item.lane.type===1?(item.carRecord.epicture !== null?item.carRecord.epicture:src):(item.carRecord.lpicture!==null?item.carRecord.lpicture:src)):src" :preview-src-list="item.carRecord!==null?(item.lane.type===1?(item.carRecord.epicture !== null?item.carRecord.epicture:src):(item.carRecord.lpicture!==null?item.carRecord.lpicture:src)):src">
              <div slot="placeholder" class="image-slot">
                加载中<span class="dot">...</span>
              </div>
            </el-image>
